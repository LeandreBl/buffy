...
Test(buffy, peek_from_empty_buffer)
{
    struct buffy buffer;
    size_t capacity = 0;
    ...
    cr_assert_eq(buffy_peek(&buffer, tmp_buffer, size), 0);
    cr_assert_eq(buffer.size, 0);
    cr_assert_eq(buffer.capacity, 0);
}

Test(buffy, peek_from_too_short_buffer)
{
    struct buffy buffer;
    size_t capacity = 0;
    const char data[] = "Hello, world!";
    ...
    cr_assert_eq(buffy_peek(&buffer, tmp_buffer, size), size / 2);
    cr_assert(buffer.size == size / 2);
}

Test(buffy, peek_from_full_buffer)
{
    struct buffy buffer;
    char letter = 'a';
    ...
    for (size_t i = 0; i < 1000; i++) {
        cr_assert_eq(buffy_peek(&buffer, &letter, 1), 1);
        cr_assert_eq(letter, 'a');
        cr_assert(buffer.size == 1000);
        cr_assert(buffer.capacity >= 1000);
    }
}